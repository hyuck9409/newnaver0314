...
public class Ex1MysqlSawon {
	...
	{
		//URL,user명과,비밀번호 이 맞으면 Connection 을 얻는다
		...
		try {
			...
			stmt=conn.createStatement();//sql 을 실행하기 위한 statement 얻기
			rs=stmt.executeQuery(sql);//select 문일경우는 무조건 executeQuery 로 실행한다
			//rs.next() 는 다음 레코드로 이동을 하면서 true반환, 그런데 이동할 데이터가 더이상 없으면 false로 반환
			while(rs.next())
			{
